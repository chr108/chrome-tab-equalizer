...
const sendMessageToOffscreen = <T>(message: unknown): Promise<T> =>
...
const readSettings = async (): Promise<EqSettings> => {
  const result = await chrome.storage.local.get(STORAGE_KEY);
  return normalizeSettings((result[STORAGE_KEY] as Partial<EqSettings> | undefined) ?? DEFAULT_SETTINGS);
};

const writeSettings = async (settings: EqSettings) => {
...
const startForActiveTab = async (settings: EqSettings) => {
  ...
  (async () => {
    if (message.type === 'INIT_ACTIVE_TAB') {
      ...
      try {
        ...
        const response: InitResponse = { ok: true, settings };
    ...
    if (message.type === 'UPDATE_SETTINGS') {
      const settings = normalizeSettings(message.settings as Partial<EqSettings>);
